...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from,
    ...
    # TODO: trainCameras.original_image
    parser = ArgumentParser(description="Training script parameters")
    lp = ModelParams(parser)
    with torch.no_grad():
        dataset_gt = dataset
        gaussians_gt = scene.gaussians
        background = torch.tensor(bg_color, dtype=torch.float32, device="cuda")
        views = scene.getTrainCameras()
        for i, view in enumerate(views):
            render_gt = render(view, gaussians_gt, pipe, background, kernel_size=dataset_gt.kernel_size)["render"]
            render_gt = render_gt.cuda()
            #resize
            target_size = scene.train_cameras[1.0][i].original_image.size()
            render_gt = F.interpolate(render_gt.unsqueeze(0), size = (target_size[1],target_size[2]), mode='bilinear', align_corners=False)
            scene.train_cameras[1.0][i].original_image = render_gt.squeeze(0)
    ########################################################
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if dataset.ray_jitter:
            subpixel_offset = torch.rand((int(viewpoint_cam.image_height), int(viewpoint_cam.image_width), 2),
        ...
        # print(gt_image.size())#torch.Size([3, 411, 618])
